...
build = '{}'.format(info["build"])
script_version = '{}'.format(info["script_version"])
window_title = f"Cloudflare WARP+"
os.system('title ' + window_title if os.name == 'nt' else 'PS1="\[\e]0;' + window_title + '\a\]"; echo $PS1')
os.system('cls' if os.name == 'nt' else 'clear')
print ("[+] ABOUT SCRIPT:")
print ("[-] With this script, you can getting Unlimited GB on WARP+.")
print (f"[-] Version: {script_version} ({build})")
print ("------------------------------------------------------------")
print ("[#] Copy ID From: Settings/Advanced/Diagnostics/ID")
print ("------------------------------------------------------------")
...
def genString(stringLength):
	try:
		...
		print(error)		    	
url = f'https://api.cloudflareclient.com/v0a{build}/reg'
def run():
	try:
